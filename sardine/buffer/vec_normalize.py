...
class VecNormalize(VecEnvWrapper):
    ...
    def __init__(
        ...
        if self.norm_obs:
            ...
            if isinstance(self.observation_space, spaces.Dict):
                ...
                self.obs_rms = utils.RunningMeanStd(shape=self.observation_space.shape)
                # Update observation space when using image
                # See GH #1214
                ...
                if utils.is_image_space(self.observation_space):
    ...
    def _sanity_checks(self) -> None:
        """
        Check the observations that are going to be
        normalized are of the correct type (spaces.Box).
        """
        if isinstance(self.observation_space, spaces.Dict):
            ...
            for obs_key in self.norm_obs_keys:
                if not isinstance(self.observation_space.spaces[obs_key], spaces.Box):
                    raise ValueError(
                        f"VecNormalize only `spaces.Box` spaces but {obs_key} "
            ...
            if self.norm_obs_keys is not None:
                raise ValueError(
                    "`norm_obs_keys` ,is applicable only with `gym.spaces.Dict`spaces"
                )

        else:
            raise ValueError(
                "VecNormalize only supports `spaces.Box` and `spaces.Dict` obs spaces, "
    ...
    def _normalize_obs(
        self, obs: np.ndarray, obs_rms: utils.RunningMeanStd
    ) -> np.ndarray:
        """
