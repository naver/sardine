...
class BaseBuffer(ABC):
    ...
    def __init__(
        ...
        self.pos = 0
        self.traj_pos = np.zeros((n_envs,), dtype = int)
        self.traj_start = np.zeros((n_envs,), dtype = int)
        self.full = False
    ...
    def to_torch(self, array: np.ndarray, copy: bool = True) -> th.Tensor:
        ...
        changing things by reference).
        This argument is inoperative if the device is not the CPU.
        :return:
        """
        if copy:
...
class ReplayBuffer(BaseBuffer):
    ...
    def __init__(
        ...
        super().__init__(
            buffer_size, observation_space, action_space, is_pomdp = False, device = device, n_envs=n_envs
        )
        ...
        if psutil is not None:
            ...
            if total_memory_usage > mem_available:
                ...
                mem_available /= 1e9
                warnings.warn(
                    "This system does not have apparently enough"
                    + "memory to store the complete "
    ...
    def add(
        ...
        if self.handle_timeout_termination:
            self.timeouts[self.pos] = np.array(infos.get("TimeLimit.truncated", False))
...
class RolloutBuffer(BaseBuffer):
    ...
    Hence, it is only involved in policy and value function
    training but not action selection.
    :param buffer_size: Max number of element in the buffer
    :param observation_space: Observation space
    :param action_space: Action space
    :param device: PyTorch device
    :param gae_lambda: Factor for trade-off of bias vs variance for
    Generalized Advantage Estimator Equivalent to classic advantage when set to 1.
    ...
    def compute_returns_and_advantage(
        ...
        to compute the advantage. To obtain Monte-Carlo advantage estimate
        (A(s) = R - V(S))
        where R is the sum of discounted reward with value bootstrap
        (because we don't always have full episode), set ``gae_lambda=1.0``
...
class DictReplayBuffer(ReplayBuffer):
    ...
    :param optimize_memory_usage: Enable a memory efficient variant
        Disabled for now
    (see https://github.com/DLR-RM/stable-baselines3/pull/243#discussion_r531535702)
    :param handle_timeout_termination: Handle timeout termination (due to timelimit)
        separately and treat the task as infinite horizon task.
        https://github.com/DLR-RM/stable-baselines3/issues/284
    """
    ...
    def _get_samples(
        ...
        # (we are using only one env for now)
...
class DictRolloutBuffer(RolloutBuffer):
    ...
    :param device: PyTorch device
    :param gae_lambda: Factor for trade-off of bias vs variance for
    Generalized Advantage Estimator Equivalent to Monte-Carlo advantage
    estimate when set to 1.
    :param gamma: Discount factor
    :param n_envs: Number of parallel environments
    """
    ...
    def get(
        ...
        if not self.generator_ready:
            ...
            _tensor_names = ["actions", "values", "log_probs", "advantages", "rewards"]
...
class POMDPDictReplayBuffer(DictReplayBuffer):
    '''
        Buffer storing full trajectories for Off-Policy POMDP learning with Dict observations, inspired by
        https://github.com/twni2016/pomdp-baselines/blob/main/buffers/seq_replay_buffer_efficient.py
    '''
    def __init__(
        self,
        buffer_size: int,
        ...
        # NOTE: valid_starts are (internal) masks which is 1
        # 	if we can SAMPLE the (sub)sequence FROM this index else 0.
        # For each trajectory, the first indices are 1s, and the LAST sampled_seq_len indices are 0s
        # 	That is to say, if its length < sampled_seq_len, then the valid_starts looks like 000000000
        # 	else looks like 11111000000 (have sampled_seq_len - 1 zeros)
        self._valid_starts = np.zeros((self.buffer_size, self.n_envs), dtype=bool)
    ...
    def add(
        ...
        if self.pos == self.buffer_size:
            ...
            for e in range(self.n_envs):
                self._valid_starts[self.traj_start[e]: self.traj_start[e] + self.traj_pos[e] + 1 - self._sampled_seq_len, e] = 1
            self.traj_pos = np.zeros((self.n_envs,), dtype = int)
            self.traj_start = np.zeros((self.n_envs,), dtype = int)
        elif 1 in done:
            done_envs = np.nonzero(done)[0]
            for e in done_envs:
                self._valid_starts[self.traj_start[e] : self.traj_start[e] + self.traj_pos[e] + 1 - self._sampled_seq_len, e] = 1
                self.traj_start[e] = self.pos
                self.traj_pos[e] = 0
    ...
    def sample(
        ...
        :return:
        """
        valid_inds = np.nonzero(self._valid_starts)
        random_valid_inds = np.random.randint(len(valid_inds[0]), size = (batch_size,))
        batch_inds = valid_inds[0][random_valid_inds]
        env_inds = valid_inds[1][random_valid_inds]
    ...
    def _get_samples(
        ...
        ## Only for vector observations for now !!

        env_indices = np.repeat(env_inds, self._sampled_seq_len)
        inds = np.tile(np.arange(self._sampled_seq_len), len(batch_inds)) + np.repeat(batch_inds, self._sampled_seq_len)

        obs_ = self._normalize_obs(
            {
                key: obs[inds, env_indices].reshape(len(batch_inds), self._sampled_seq_len, -1)
                for key, obs in self.observations.items()
            },
            env,
        )
        ...
        return DictReplayBufferSamples(
            observations=observations,
            actions=self.to_torch(self.actions[inds, env_indices]).reshape(len(batch_inds), self._sampled_seq_len, -1),
            next_observations=next_observations,
            dones=self.to_torch(
                self.dones[inds, env_indices].reshape(len(batch_inds), self._sampled_seq_len)
            ),
            rewards=self.to_torch(
                self._normalize_reward(
                    self.rewards[inds, env_indices].reshape(len(batch_inds), self._sampled_seq_len), env
                )
            ),
        )
